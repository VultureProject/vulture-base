...
if [ $OS = "FreeBSD" ]; then
    PATH="${PATH}:/usr/local/bootstrap-openjdk11/bin"
fi
...
if [ "$OS" = "Darwin" ]; then
  ...
  exec java ${JMEM} ${JAVAGC} -Xdock:icon="../Resources/ZAP.icns" -jar "${BASEDIR}/zap-2.9.0.jar" "${ARGS[@]}"
else
  exec java ${JMEM} ${JAVAGC} ${JAVADEBUG} -jar "${BASEDIR}/zap-2.9.0.jar" "${ARGS[@]}"
fi