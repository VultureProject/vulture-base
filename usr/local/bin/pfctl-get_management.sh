...
# Also create the masquerading.ip if file does not exists
if ! [ -f /usr/local/etc/masquerading.ip ] ; then
    cp -f /usr/local/etc/management.ip /usr/local/etc/masquerading.ip
    /usr/sbin/chown root:wheel /usr/local/etc/masquerading.ip
    /bin/chmod 644 /usr/local/etc/masquerading.ip
fi
